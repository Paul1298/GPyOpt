...
class BayesianOptimization(BO):
    ...
    :param domain: list of dictionaries containing the description of the inputs variables (See MyGPyOpt.core.task.space.Design_space class for details).
    :param constraints: list of dictionaries containing the description of the problem constraints (See MyGPyOpt.core.task.space.Design_space class for details).
    ...
    def __init__(self, f, domain=None, constraints=None, cost_withGradients=None, model_type='GP', X=None, Y=None,
                 ...
                 batch_size=1, num_cores=1, verbosity=False, verbosity_model=False, maximize=False,
                 de_duplication=False, **kwargs):
        ...
        # --- CHOOSE the model type. If an instance of a MyGPyOpt model is passed (possibly user defined), it is used.
        self.model_type = model_type
        self.exact_feval = exact_feval  # note that this 2 options are not used with the predefined model
        self.normalize_Y = normalize_Y

        if 'model' in self.kwargs:
            if isinstance(kwargs['model'], MyGPyOpt.models.base.BOModel):
        ...
        if 'acquisition' in self.kwargs:
            if isinstance(kwargs['acquisition'], MyGPyOpt.acquisitions.AcquisitionBase):
        ...
        super(BayesianOptimization, self).__init__(space=self.space,
    ...
    def _evaluator_chooser(self):
        return self.problem_config.evaluator_creator(self.evaluator_type, self.acquisition, self.batch_size,
