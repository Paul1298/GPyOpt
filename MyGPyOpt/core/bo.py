...
class BO(object):
    ...
    :param model: MyGPyOpt model class.
    :param space: MyGPyOpt space class.
    :param objective: MyGPyOpt objective class.
    :param acquisition: MyGPyOpt acquisition class.
    :param evaluator: MyGPyOpt evaluator class.
    ...
    :param cost: MyGPyOpt cost class (default, none).
    :param normalize_Y: whether to normalize the outputs before performing any optimization (default, True).
    :param model_update_interval: interval of collected observations after which the model is updated (default, 1).
    :param de_duplication: MyGPyOpt DuplicateManager class. Avoids re-evaluating the objective at previous, pending or infeasible locations (default, False).
    """
    ...
    def run_optimization(self, max_iter=0, max_time=np.inf, eps=1e-8, context=None, verbosity=False,
        ...
        if self.save_models_parameters == True:
            if not (isinstance(self.model, MyGPyOpt.models.GPModel) or isinstance(self.model,
                                                                                  MyGPyOpt.models.GPModel_MCMC)):
    ...
    def _print_convergence(self):
        ...
        if self.verbosity:
            ...
            if self.initial_iter:
                print('** MyGPyOpt Bayesian Optimization class initialized successfully **')
    ...
    def save_report(self, report_file=None):
        ...
        with open(report_file, 'w') as file:
            ...
            file.write(
                '-----------------------------' + ' MyGPyOpt Report file ' + '-----------------------------------\n')
            file.write('MyGPyOpt Version ' + str(MyGPyOpt.__version__) + '\n')
