...
import MyGPyOpt
from MyGPyOpt.util.general import samples_multidimensional_uniform
...
class TestAcquisitions(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.problem_config = {
            'objective': MyGPyOpt.objective_examples.experimentsNd.alpine1(input_dim = input_dim).f,
        ...
        feasible_region = MyGPyOpt.Design_space(space = self.problem_config['domain'], constraints = self.problem_config['constraints'])
        self.f_inits = MyGPyOpt.experiment_design.initial_design('random', feasible_region, 5)
